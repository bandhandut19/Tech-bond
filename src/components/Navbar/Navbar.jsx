...
const Navbar = () => {
  ...
  return (
    <div
      className={`navbar ${darkMode == "dark" ? "text-white" : "text-black"}`}
    >
      <div className="navbar-start">
        <div className="dropdown">
          ...
          >
            ...
            {user ? (
              ...
              <li>
                <button onClick={handleAddProduct}>My Cart</button>
              </li>
            )}
          </ul>
        </div>
        <div className="flex text-center items-center justify-center">
          ...
          <Link
            to="/"
            className="normal-case md:text-4xl lg:block hidden md:block text-lg font-extrabold text-amber-600"
          >
            TechBond
          </Link>
        </div>
          ...
          {user ? (
            <>
              <div className="flex items-center flex-col-reverse justify-center gap-2">
                <div>
                  <img
                    className="w-[3rem] btn-circle"
                    src={
                      user?.photoURL ? user?.photoURL : currentUserInfo?.photo
                    }
                    alt=""
                  />
                </div>
                <div>
                  <h1 className="bg-amber-500 px-2 rounded ">
                    {user?.displayName
                      ? user?.displayName
        ...
        <div>
          {user ? (
            <Link
              ...
              className="px-3 md:py-0.5 md:ml-0 ml-6 rounded font-bold bg-amber-600"
            >
              Logout
            </Link>
          ) : (
          ...
          <button
            onClick={modeHandler}
            className="px-2  ml-2 py-2  rounded-full font-bold bg-amber-600"
          >
            {darkMode == "light" ? (
